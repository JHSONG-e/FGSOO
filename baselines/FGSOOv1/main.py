...
for seed in range(num_exp):
    ...
    while (begin_i < budget):
        point, curr_domain = algo.pull(begin_i, method, dim, func, kernel_type, init_num)
        # print(point)
        ...
        if method =='FGSOOv1':
            ...
            if( end_i + BO_iteration <budget):
                reward = Baye(point,torch.tensor(curr_domain,dtype=torch.float64), dim, func, kernel_type,BO_iteration ,init_num )
                end_i += BO_iteration
