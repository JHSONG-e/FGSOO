...
def Baye(point,value,bounds, dim, func, kernel_type, BO_iter,init_num=5):
    # bounds_nor = torch.tensor([[-1., 1.]] * dim).double()
    bounds_nor = torch.tensor([[0., 1.]] * dim,dtype = torch.float64)
    dataset = init_points_dataset_bo(
    ...
    dataset = update_dataset_ucb(point, init_x, value, dataset)
    # print(dataset)
    for i in range(BO_iter - 5):
        #beta越大，越是探索
        _, next_x = next_point_bo(dataset, 0.2 * dim * torch.log(torch.tensor(2 * dataset['f'].shape[0])) , bounds_nor, kernel_type)
        # print(next_x)
